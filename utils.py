...
def load_datasets(splits):
    ...
    for split in splits:
        ...
        # scene_num = -1
        if len(components) > 1:
            split, number = components[0], components[1]
            if '-' in number:
                start_num, end_num = number.split('-')
                start_num = int(start_num)
                end_num = int(end_num)
            # elif number.startwith('s'):
            #     start_num = -1
            #     end_num = -1
            #     scene_num = int(number[1:])
            else:
                start_num = 0
                end_num = int(number)
        
        path_ids = None
        if os.path.isfile('data/R2R_train_enc_{}shot.jsonl'.format(number)):
            import jsonlines
            with jsonlines.open('data/R2R_train_enc_{}shot.jsonl'.format(number)) as reader:
                path_ids = [item['instr_id'].split('_')[0] for item in reader]
                path_ids = list(set(path_ids))
        
        # if os.path.isfile('data/R2R_train_enc_{}scene.jsonl'.format(scene_num)):
        #     import jsonlines
        #     with jsonlines.open('data/R2R_train_enc_{}scene.jsonl'.format(scene_num)) as reader:
        #         path_ids = [item['instr_id'].split('_')[0] for item in reader]
        #         path_ids = list(set(path_ids))
        ...
        if int(number) > 0 and path_ids is not None:
            new_data = [item for item in new_data if str(item['path_id']) in path_ids]
        elif int(number) > 0:
            random.seed(0)              # Make the data deterministic, additive
            random.shuffle(new_data)
            new_data = new_data[:int(number)]

        # # Partition
        # if start_num >= 0:
        #     random.seed(0)              # Make the data deterministic, additive
        #     random.shuffle(new_data)
        #     print("Loading %d-%d data from %s" % (start_num, end_num, split))
        #     new_data = new_data[start_num:end_num]
...
def load_cvdn_datasets(splits):
    data = []
    for split in splits:
        assert split in ['train', 'val_seen', 'val_unseen', 'test']
        with open('data/ndh_data/tasks/NDH/data/%s.json' % split) as f:
            data += json.load(f)
    return data
...
def read_img_features(feature_store, test_only=False):
    ...
    if not test_only:
        if feature_store.endswith('.tsv'):
            ...
            features = np.load(feature_store, allow_pickle=True).item()
    else:
        features = None
...
def get_point_angle_feature(baseViewId=0):
    ...
    for ix in range(36):
        ...
        state = sim.getState()
        assert state[0].viewIndex == ix

        heading = state[0].heading - base_heading

        feature[ix, :] = angle_feature(heading, state[0].elevation)
    return feature
...
def clip_tokenize(texts, context_length = 77, truncate = False, _tokenizer=None):
    """
